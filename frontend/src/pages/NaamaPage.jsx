...
import { runNaamaAgent } from '@/functions/runNaamaAgent';
...
export default function NaamaPage() {
  ...
  const [expandedJobs, setExpandedJobs] = useState(() => {
    try {
      const saved = localStorage.getItem('naama_expanded_jobs');
  ...
  const [matchScoreFilter, setMatchScoreFilter] = useState(() => localStorage.getItem('naama_matchScoreFilter') || "80+");
  const [priorityFilter, setPriorityFilter] = useState(() => localStorage.getItem('naama_priorityFilter') || "all");
  const [showAllMatches, setShowAllMatches] = useState(() => localStorage.getItem('naama_showAllMatches') === 'true');
  const [showFullMatchOnly, setShowFullMatchOnly] = useState(() => localStorage.getItem('naama_showFullMatchOnly') === 'true');
  const [showBestFitOnly, setShowBestFitOnly] = useState(() => localStorage.getItem('naama_showBestFitOnly') === 'true');
  const [showRecentCvsOnly, setShowRecentCvsOnly] = useState(() => localStorage.getItem('naama_showRecentCvsOnly') === 'true');
  const [handledFilter, setHandledFilter] = useState(() => localStorage.getItem('naama_handledFilter') || "all");
  const [showWithTasksOnly, setShowWithTasksOnly] = useState(false);

  useEffect(() => { localStorage.setItem('naama_matchScoreFilter', matchScoreFilter); }, [matchScoreFilter]);
  useEffect(() => { localStorage.setItem('naama_priorityFilter', priorityFilter); }, [priorityFilter]);
  useEffect(() => { localStorage.setItem('naama_handledFilter', handledFilter); }, [handledFilter]);
  useEffect(() => { localStorage.setItem('naama_showFullMatchOnly', showFullMatchOnly); }, [showFullMatchOnly]);
  useEffect(() => { localStorage.setItem('naama_showBestFitOnly', showBestFitOnly); }, [showBestFitOnly]);
  useEffect(() => { localStorage.setItem('naama_showRecentCvsOnly', showRecentCvsOnly); }, [showRecentCvsOnly]);
  useEffect(() => { localStorage.setItem('naama_showAllMatches', showAllMatches); }, [showAllMatches]); // "all", "handled", "unhandled"
  ...
  const [viewMode, setViewMode] = useState(() => localStorage.getItem('naama_viewMode') || "table");

  useEffect(() => { localStorage.setItem('naama_viewMode', viewMode); }, [viewMode]); // "grouped" or "table"
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Load primary data first (matches, jobs, candidates)
      ...
      // If "show all" mode is active, don't filter anything
      if (showAllMatches) {
        setMatches(matchList);
      } else {
        const candidateStatusMap = new Map(candidateList.map(c => [c.id, c.status]));
        const naamaJobIds = new Set(
          jobList
            .filter(job => job.assigned_agent === 'naama')
            .map(job => job.id)
        );
        const filteredMatchList = matchList.filter(m => {
          const candidateStatus = candidateStatusMap.get(m.candidate_id);
          if (candidateStatus === "לא רלוונטי יותר" || candidateStatus === "לא מתאים - נסגר") return false;
          if (!naamaJobIds.has(m.job_id)) return false;
          return true;
        });
  ...
  const loadAgentStatus = async () => {
    try {
      ...
      if (statuses.length > 0) {
        ...
        // Auto-release focus if agent finished autonomously
        if (status.focused_job_id && status.focus_end_time && !status.is_running) {
          console.log('Agent finished focus autonomously - releasing focus');
          ...
          const updatedStatuses = await base44.entities.AgentRunStatus.filter({ agent_name: 'naama' });
  ...
  const carmitAssignedJobs = useMemo(() => {
    return jobs.filter(j => j.status === 'פעילה' && j.assigned_agent === 'naama');
  }, [jobs]);
  ...
  const handleRunAgent = async () => {
    ...
    try {
      ...
      console.error('Error running naama agent:', error);
  ...
  const handleSetFocus = async (job) => {
    ...
    try {
      ...
      toast.success(`נעמה ממוקדת על: ${job.title}`);
  ...
  const handleCancelFocus = async () => {
    ...
    try {
      if (agentStatus) {
        ...
        toast.success("המיקוד של נעמה בוטל");
        loadAgentStatus();
      }
    } catch (error) {
  ...
  const handleRevalidateMatches = async () => {
    if (!confirm('לבדוק מחדש את ההתאמות הקיימות של נעמה? התהליך יעדכן תיאורים וימחק התאמות שכבר לא מתאימות (10 התאמות בכל לחיצה)')) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const unsubscribeJob = base44.entities.Job.subscribe((event) => {
      // Only reload jobs, not everything
      setJobs(prev => {
  ...
  const handleMarkCandidateIrrelevant = async (match) => {
    setConfirmDialog({
      ...
      message: `האם לסמן את ${match.candidate_name} כ"לא מתאים - נסגר"? המועמד יוסר לחלוטין מכל ההתאמות העתידיות במערכת - לא רק אצל נעמה אלא אצל כל הסוכנים (רועי, רמי, אליק, איתי, ליאור, אופיר, GC).`,
      confirmText: "סמן כלא מתאים",
      variant: "destructive",
      onConfirm: async () => {
        try {
          // Save current view settings to localStorage BEFORE loadData resets them
          localStorage.setItem('naama_viewMode', viewMode);
          localStorage.setItem('naama_expanded_jobs', JSON.stringify(expandedJobs));
          ...
          base44.entities.MatchNote.create({
            ...
            note_text: `המועמד סומן כ"לא מתאים - נסגר" על ידי נעמה ולא יופיע בהתאמות עתידיות של כל הסוכנים`,
            is_system_note: true
  ...
  // For each match, find if there's a better match (higher score) for the same candidate on a different job
  const betterMatchByMatchId = useMemo(() => {
    ...
    matches.forEach(match => {
      // Find all other matches for same candidate with higher score on a different job
  ...
  // "Handled" indicators for green row highlight
  ...
  // O(n) instead of O(n*m) - build job title lookup first
  ...
  const filteredMatches = useMemo(() => {
    ...
    return matches.filter(match => {
      // In "show all" mode, skip irrelevant candidate filtering
      if (!showAllMatches) {
      ...
      // Apply best fit filter - O(1) lookup using pre-computed map
      let bestFitFilter = true;
      if (showBestFitOnly) {
        bestFitFilter = bestMatchIdByCandidate.get(match.candidate_id)?.id === match.id;
      }

      // Apply recent CVs filter - only candidates whose CV arrived in last 10 days
  ...
  }, [matches, candidatesMap, searchTerm, matchScoreFilter, priorityFilter, jobsMap, showAllMatches, showFullMatchOnly, showBestFitOnly, showRecentCvsOnly, showWithTasksOnly, tasksCountByMatch, rotemTasks, bestMatchIdByCandidate]);
  ...
  const matchesByJob = useMemo(() => {
    ...
    // Add matches ONLY for jobs assigned to Naama by Carmit
  ...
  const toggleJobExpand = (jobId) => {
    setExpandedJobs(prev => {
      ...
      localStorage.setItem('naama_expanded_jobs', JSON.stringify(newState));
      return newState;
    });
  };

  if (loading || !user) {
    return <LoadingSpinner message="טוען דף נעמה..." />;
  }
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      <MatchesLoadingToast isLoading={loading} agentName="נעמה" />
      <div className="flex flex-col gap-3">
        <div>
          <h1 className="text-xl md:text-3xl font-bold text-gray-800">נעמה - מומחית תוכנה</h1>
          <p className="text-xs md:text-base text-gray-600">סוכנת AI להתאמות בין מועמדים למשרות תוכנה</p>
        </div>
      </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div className="flex items-center gap-2 md:gap-4">
              <img 
                ...
                alt="נעמה" 
                className="w-12 h-12 md:w-16 md:h-16 rounded-full object-cover border-2 md:border-4 border-orange-200 shadow-lg flex-shrink-0"
              />
              <div className="flex-1 min-w-0">
                <CardTitle className="flex items-center gap-2 flex-wrap text-base md:text-lg">
                  <span className="truncate">נעמה - תוכנה</span>
                ...
                <div className="mt-2 flex gap-1 md:gap-3 flex-wrap">
                  {(() => {
                    ...
                    const naamaMatches = matches.filter(m => m.match_score >= 90);
                    
                    const todayCount = naamaMatches.filter(m => 
                      new Date(m.created_date) >= todayStart
                    ).length;
                    
                    const weekCount = naamaMatches.filter(m => 
                      new Date(m.created_date) >= weekAgo
                    ).length;
                    
                    const monthCount = naamaMatches.filter(m => 
      ...
      {/* Button Navigation - דואר נכנס / יוצא */}
          ...
          <Card className="bg-gradient-to-r from-purple-50 to-pink-50 border-purple-200">
            <CardHeader>
              <CardTitle className="flex items-center gap-3">
                ...
                />
                <div>
                  <span className="text-lg">כרמית הקצתה לנעמה</span>
                  <p className="text-sm text-gray-600 font-normal">משרות תוכנה לטיפול</p>
                </div>
              </CardTitle>
            </CardHeader>
            <CardContent>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <Briefcase className={`w-4 h-4 ${isFocused ? 'text-orange-600' : 'text-gray-500'}`} />
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-2">
            <Button
              ...
              className={`flex items-center gap-2 ${viewMode === "grouped" ? 'bg-orange-600 hover:bg-orange-700' : ''}`}
            >
            ...
            <Button
              ...
              className={`flex items-center gap-2 ${viewMode === "table" ? 'bg-orange-600 hover:bg-orange-700' : ''}`}
            >
                                 ...
                                 return (
                                   <Popover>
                                     <PopoverTrigger asChild onClick={(e) => e.stopPropagation()}>
                                       <button className="text-orange-700 hover:text-orange-900 underline decoration-dotted font-semibold text-right">
                                         {jobGroup.job_title}
                                       </button>
                                     </PopoverTrigger>
                    ...
                    <CardContent className="p-0">
                      {jobGroup.matches.length === 0 ? (
                        <div className="p-6 text-center text-gray-500">
                          ...
                          <p>נעמה עדיין לא עבדה על משרה זו</p>
                                   ...
                                   <DropdownMenuContent 
                                     ...
                                     sideOffset={8}>
                                      ...
                                      <DropdownMenuItem onClick={() => setAgentFeedbackDialog({ isOpen: true, match })}>
                                        <Bot className="w-4 h-4 text-orange-600 shrink-0 ml-2" />
                                        שיחה עם נעמה
                                      </DropdownMenuItem>
