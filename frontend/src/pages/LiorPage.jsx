...
import { runLiorAgent } from '@/functions/runLiorAgent';
...
import { User as UserIcon, MoreHorizontal, PlusSquare, ClipboardList, BrainCircuit } from 'lucide-react';
...
export default function LiorPage() {
  ...
  const [expandedJobs, setExpandedJobs] = useState(() => {
    try {
      const saved = localStorage.getItem('lior_expanded_jobs');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [matchList, jobList, candidateList] = await Promise.all([
        base44.entities.Match.filter({ user_name: { $regex: 'ליאור' } }, '-created_date'),
      ...
      // Filter out matches for irrelevant candidates AND misaligned jobs
      const candidateStatusMap = new Map(candidateList.map(c => [c.id, c.status]));
      
      // Determine which jobs belong to Lior
      const liorJobIds = new Set(
        jobList
          .filter(job => {
            const searchText = `${job.title} ${job.description} ${job.requirements}`.toLowerCase();
            return searchText.includes('מערכת') || 
                   searchText.includes('system engineer') ||
                   searchText.includes('srs') || 
                   searchText.includes('mbse') ||
                   searchText.includes('doors');
          })
          .map(job => job.id)
      );
      ...
      const filteredMatchList = matchList.filter(m => {
        ...
        if (candidateStatusMap.get(m.candidate_id) === "לא רלוונטי יותר") {
          return false;
        }
        // Filter jobs that don't belong to Lior
        if (!liorJobIds.has(m.job_id)) {
          return false;
        }
        return true;
      });
  ...
  const loadAgentStatus = async () => {
    try {
      const statuses = await base44.entities.AgentRunStatus.filter({ agent_name: 'lior' });
  ...
  // Calculate Carmit's job assignments for Lior - using actual DB assignment
  const carmitAssignedJobs = useMemo(() => {
    // CRITICAL: Show only jobs that Carmit actually assigned to Lior in the database
    // This matches the logic in runLiorAgent which filters by assigned_agent === 'lior'
    return jobs.filter(j => j.status === 'פעילה' && j.assigned_agent === 'lior');
  }, [jobs]);

  const handleRunAgent = async () => {
    if (runningAgent) return; // Prevent double-clicks
    
    setRunningAgent(true);
    try {
      await runLiorAgent({});
      toast.success('ליאור התחיל לרוץ');
      setTimeout(() => {
        loadAgentStatus();
      }, 2000);
    } catch (error) {
      console.error('Error running lior agent:', error);
      if (error.message?.includes('Rate limit')) {
        toast.error('יותר מדי בקשות - אנא המתן מספר שניות ונסה שוב');
      } else {
  ...
  const handleSetFocus = async (job) => {
    ...
    try {
      ...
      if (statuses.length > 0) {
        ...
        const newStatus = await base44.entities.AgentRunStatus.create({
          agent_name: 'lior',
          ...focusData
        });
      ...
      toast.success(`ליאור ממוקד על: ${job.title}`);
    } catch (error) {
  ...
  const handleCancelFocus = async () => {
    ...
    try {
      if (agentStatus) {
        ...
        toast.success("המיקוד של ליאור בוטל");
        loadAgentStatus();
      }
    } catch (error) {
  ...
  const handleRevalidateMatches = async () => {
    if (!confirm('לבדוק מחדש את ההתאמות הקיימות של ליאור? התהליך יעדכן תיאורים וימחק התאמות שכבר לא מתאימות (10 התאמות בכל לחיצה)')) {
      return;
    }
  ...
  const handleMarkCandidateIrrelevant = async (match) => {
    setConfirmDialog({
      ...
      message: `האם לסמן את ${match.candidate_name} כ"לא מתאים - נסגר"? המועמד יוסר לחלוטין מכל ההתאמות העתידיות במערכת - לא רק אצל ליאור אלא אצל כל הסוכנים (נעמה, רועי, רמי, אליק, איתי, אופיר, GC).`,
      confirmText: "סמן כלא מתאים",
      variant: "destructive",
      onConfirm: async () => {
        try {
          localStorage.setItem('lior_viewMode', viewMode);
          localStorage.setItem('lior_expanded_jobs', JSON.stringify(expandedJobs));
          ...
          base44.entities.MatchNote.create({
            ...
            note_text: `המועמד סומן כ"לא מתאים - נסגר" על ידי ליאור ולא יופיע בהתאמות עתידיות של כל הסוכנים`,
            is_system_note: true
  ...
  const matchesByJob = useMemo(() => {
    ...
    // CRITICAL: Start with ALL Carmit-assigned jobs (even those with 0 matches)
    ...
    // Add matches for jobs assigned to Lior by Carmit
    ...
    // Sort: Jobs with matches first, then by match count
  ...
  }, [filteredMatches, displayLimit, carmitAssignedJobs]);
  ...
  const toggleJobExpand = (jobId) => {
    setExpandedJobs(prev => {
      ...
      localStorage.setItem('lior_expanded_jobs', JSON.stringify(newState));
      return newState;
    });
  };

  if (loading) {
    return <LoadingSpinner message="טוען דף ליאור..." />;
  }
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      <div className="flex flex-col gap-3">
        <div>
          <h1 className="text-xl md:text-3xl font-bold text-gray-800">ליאור - מומחה הנדסת מערכת</h1>
          <p className="text-xs md:text-base text-gray-600">סוכן AI להתאמות מועמדים למשרות הנדסת מערכת</p>
        </div>
      </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div className="flex items-center gap-2 md:gap-4">
              <img 
                ...
                alt="ליאור" 
                className="w-12 h-12 md:w-16 md:h-16 rounded-full object-cover border-2 md:border-4 border-amber-200 shadow-lg flex-shrink-0"
              />
              <div className="flex-1 min-w-0">
                <CardTitle className="flex items-center gap-2 flex-wrap text-base md:text-lg">
                  <span className="truncate">ליאור - הנדסת מערכת</span>
                ...
                <div className="mt-2 flex gap-1 md:gap-3 flex-wrap">
                  {(() => {
                    ...
                    const liorMatches = matches.filter(m => m.match_score >= 85);
                    
                    return (
                      <>
                        <Badge className="bg-green-100 text-green-800 text-xs">
                          <TrendingUp className="w-2 h-2 md:w-3 md:h-3 ml-1" />
                          <span className="hidden sm:inline">היום:</span> {liorMatches.filter(m => new Date(m.created_date) >= todayStart).length}
                        </Badge>
                        <Badge className="bg-blue-100 text-blue-800 text-xs">
                          <Activity className="w-2 h-2 md:w-3 md:h-3 ml-1" />
                          <span className="hidden sm:inline">שבוע:</span> {liorMatches.filter(m => new Date(m.created_date) >= weekAgo).length}
                        </Badge>
                        <Badge className="bg-purple-100 text-purple-800 text-xs">
                          <Calendar className="w-2 h-2 md:w-3 md:h-3 ml-1" />
                          <span className="hidden sm:inline">חודש:</span> {liorMatches.filter(m => new Date(m.created_date) >= monthAgo).length}
                        </Badge>
                      </>
                    );
                  })()}
          ...
          <Card className="bg-gradient-to-r from-purple-50 to-pink-50 border-purple-200">
            <CardHeader>
              <CardTitle className="flex items-center gap-3">
                ...
                />
                <div>
                  <span className="text-lg">כרמית הקצתה לליאור</span>
                  <p className="text-sm text-gray-600 font-normal">משרות הנדסת מערכת לטיפול</p>
                </div>
              </CardTitle>
            </CardHeader>
            <CardContent>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <Briefcase className={`w-4 h-4 ${isFocused ? 'text-amber-600' : 'text-gray-500'}`} />
      ...
      {activeTab === "outbox" && (
        <div className="space-y-6">
          ...
          <div className="flex gap-2">
            <Button
              ...
              className={`flex items-center gap-2 ${viewMode === "grouped" ? 'bg-amber-600 hover:bg-amber-700' : ''}`}
            >
            ...
            <Button
              ...
              className={`flex items-center gap-2 ${viewMode === "table" ? 'bg-amber-600 hover:bg-amber-700' : ''}`}
            >
        ...
        <UnifiedTableView
          ...
          onRefreshData={() => queryClient.invalidateQueries({ queryKey: ['lior-matches'] })}
        />
      ) : (
        <>
                                 ...
                                 return (
                                   <Popover>
                                     <PopoverTrigger asChild onClick={(e) => e.stopPropagation()}>
                                       <button className="text-amber-700 hover:text-amber-900 underline decoration-dotted font-semibold text-right">
                                         {jobGroup.job_title}
                                       </button>
                                     </PopoverTrigger>
                    ...
                    <CardContent className="p-0">
                      {jobGroup.matches.length === 0 ? (
                        <div className="p-6 text-center text-gray-500">
                          ...
                          <p>ליאור עדיין לא עבד על משרה זו</p>
                       ...
                       <TableHeader>
                         ...
                         <TableBody>
                            ...
                            return (
                            <TableRow key={match.id} className={isHandled ? 'bg-green-50 border-r-2 border-r-green-400' : !match.is_read ? 'bg-amber-50' : ''}>
                               ...
                               <DropdownMenu>
                                 ...
                                 <DropdownMenuContent className="w-52">
                                   ...
                                   <DropdownMenuItem onClick={() => setAgentFeedbackDialog({ isOpen: true, match })}>
                                     <Bot className="w-4 h-4 text-amber-600 shrink-0 ml-2" />
                                     שיחה עם ליאור
                                   </DropdownMenuItem>
