...
export default function OfirPage() {
  ...
  const [expandedJobs, setExpandedJobs] = useState(() => {
    try {
      const saved = localStorage.getItem('ofir_expanded_jobs');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [matchList, jobList, candidateList] = await Promise.all([
        base44.entities.Match.filter({ user_name: 'אופיר (סוכן AI)' }, '-created_date'),
      ...
      Promise.all([
        ...
        base44.entities.UserTask.list('-created_date', 500)
      ]).then(([notesList, whatsappList, emailList, emailOutboxList, rotemTasksList, userTasksList]) => {
        setNotes(notesList);
        setCommunications([...whatsappList, ...emailList]);
        setClientCommunications(emailOutboxList);
        setRotemTasks(rotemTasksList);
        setUserTasks(userTasksList);
      }).catch(e => console.warn('Secondary data load error:', e));

      // Determine which jobs belong to Ofir
      const ofirJobIds = new Set(
        jobList
          .filter(job => {
            const searchText = `${job.title} ${job.description} ${job.requirements}`.toLowerCase();
            return searchText.includes('מכונ') || 
                   searchText.includes('mechanical') ||
                   searchText.includes('solidworks') || 
                   searchText.includes('catia');
          })
          .map(job => job.id)
      );
      
      const filteredMatchList = matchList.filter(m => {
        // Filter jobs that don't belong to Ofir
        if (!ofirJobIds.has(m.job_id)) {
          return false;
        }
        return true;
      });
  ...
  const loadAgentStatus = async () => {
    try {
      const statuses = await base44.entities.AgentRunStatus.filter({ agent_name: 'ofir' });
  ...
  const carmitAssignedJobs = useMemo(() => {
    return jobs.filter(j => j.status === 'פעילה' && j.assigned_agent === 'ofir');
  }, [jobs]);

  const handleRunAgent = async () => {
    setRunningAgent(true);
    try {
      await base44.functions.invoke('runOfirAgent', {});
      toast.success('אופיר התחיל לרוץ');
      setTimeout(() => {
        loadAgentStatus();
      }, 2000);
    } catch (error) {
      console.error('Error running ofir agent:', error);
  ...
  const handleSetFocus = async (job) => {
    ...
    try {
      ...
      toast.success(`אופיר ממוקד על: ${job.title}`);
    } catch (error) {
  ...
  const handleCancelFocus = async () => {
    ...
    try {
      if (agentStatus) {
        ...
        toast.success("המיקוד של אופיר בוטל");
        loadAgentStatus();
      }
    } catch (error) {
  ...
  const handleRevalidateMatches = async () => {
    if (!confirm('לבדוק מחדש את ההתאמות הקיימות של אופיר? התהליך יעדכן תיאורים וימחק התאמות שכבר לא מתאימות (10 התאמות בכל לחיצה)')) {
      return;
    }
  ...
  const handleMarkCandidateIrrelevant = async (match) => {
    setConfirmDialog({
      ...
      message: `האם לסמן את ${match.candidate_name} כ"לא מתאים - נסגר"? המועמד יוסר לחלוטין מכל ההתאמות העתידיות במערכת - לא רק אצל אופיר אלא אצל כל הסוכנים (נעמה, רועי, רמי, אליק, איתי, ליאור, GC).`,
      confirmText: "סמן כלא מתאים",
      variant: "destructive",
      onConfirm: async () => {
        try {
          localStorage.setItem('ofir_viewMode', viewMode);
          localStorage.setItem('ofir_expanded_jobs', JSON.stringify(expandedJobs));
          ...
          base44.entities.MatchNote.create({
            ...
            note_text: `המועמד סומן כ"לא מתאים - נסגר" על ידי אופיר ולא יופיע בהתאמות עתידיות של כל הסוכנים`,
            is_system_note: true
  ...
  const matchesByJob = useMemo(() => {
    ...
    // Add matches ONLY for jobs assigned to Ofir by Carmit
  ...
  const toggleJobExpand = (jobId) => {
    setExpandedJobs(prev => {
      ...
      localStorage.setItem('ofir_expanded_jobs', JSON.stringify(newState));
      return newState;
    });
  };

  if (loading || !user) {
    return <LoadingSpinner message="טוען דף אופיר..." />;
  }
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      <div className="flex flex-col gap-3">
        <div>
          <h1 className="text-xl md:text-3xl font-bold text-gray-800">אופיר - מומחה הנדסת מכונות</h1>
          <p className="text-xs md:text-base text-gray-600">סוכן AI להתאמות בין מועמדים למשרות הנדסת מכונות</p>
        </div>
      </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div className="flex items-center gap-2 md:gap-4">
              <img 
                ...
                alt="אופיר" 
                className="w-12 h-12 md:w-16 md:h-16 rounded-full object-cover border-2 md:border-4 border-emerald-200 shadow-lg flex-shrink-0"
              />
              <div className="flex-1 min-w-0">
                <CardTitle className="flex items-center gap-2 flex-wrap text-base md:text-lg">
                  <span className="truncate">אופיר - הנדסת מכונות</span>
                  <Badge className="bg-emerald-100 text-emerald-800 text-xs whitespace-nowrap">
                    {carmitAssignedJobs.length} משרות
                  </Badge>
                </CardTitle>
                <p className="text-xs md:text-sm text-gray-600">
                  התאמות למשרות הנדסת מכונות
                </p>
                ...
                <div className="mt-2 flex gap-1 md:gap-3 flex-wrap">
                  {(() => {
                    ...
                    const ofirMatches = matches.filter(m => m.match_score >= 90);
                    
                    const todayCount = ofirMatches.filter(m => 
                      new Date(m.created_date) >= todayStart
                    ).length;
                    
                    const weekCount = ofirMatches.filter(m => 
                      new Date(m.created_date) >= weekAgo
                    ).length;
                    
                    const monthCount = ofirMatches.filter(m => 
          ...
          <Card className="bg-gradient-to-r from-purple-50 to-pink-50 border-purple-200">
            <CardHeader>
              <CardTitle className="flex items-center gap-3">
                ...
                />
                <div>
                  <span className="text-lg">כרמית הקצתה לאופיר</span>
                  <p className="text-sm text-gray-600 font-normal">משרות הנדסת מכונות לטיפול</p>
                </div>
              </CardTitle>
            </CardHeader>
            <CardContent>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <Briefcase className={`w-4 h-4 ${isFocused ? 'text-emerald-600' : 'text-gray-500'}`} />
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-2">
            <Button
              ...
              className={`flex items-center gap-2 ${viewMode === "grouped" ? 'bg-emerald-600 hover:bg-emerald-700' : ''}`}
            >
            ...
            <Button
              ...
              className={`flex items-center gap-2 ${viewMode === "table" ? 'bg-emerald-600 hover:bg-emerald-700' : ''}`}
            >
                                 ...
                                 return (
                                   <Popover>
                                     <PopoverTrigger asChild onClick={(e) => e.stopPropagation()}>
                                       <button className="text-emerald-700 hover:text-emerald-900 underline decoration-dotted font-semibold text-right">
                                         {jobGroup.job_title}
                                       </button>
                                     </PopoverTrigger>
                    ...
                    <CardContent className="p-0">
                      {jobGroup.matches.length === 0 ? (
                        <div className="p-6 text-center text-gray-500">
                          ...
                          <p>אופיר עדיין לא עבד על משרה זו</p>
                       ...
                       <TableHeader>
                         ...
                         <TableBody>
                           ...
                           return (
                             <TableRow key={match.id} className={isHandled ? 'bg-green-50 border-r-2 border-r-green-400' : !match.is_read ? 'bg-emerald-50' : ''}>
                                  ...
                                  <DropdownMenu>
                                    ...
                                    <DropdownMenuContent className="w-52">
                                      ...
                                      <DropdownMenuItem onClick={() => setAgentFeedbackDialog({ isOpen: true, match })}>
                                        <Bot className="w-4 h-4 text-emerald-600 shrink-0 ml-2" />
                                        שיחה עם אופיר
                                      </DropdownMenuItem>
