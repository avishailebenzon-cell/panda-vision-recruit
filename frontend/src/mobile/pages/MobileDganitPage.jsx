...
export default function MobileDganitPage() {
    ...
    const loadData = async () => {
      try {
        const [matchesData, jobsData, candidatesData, statusesData] = await Promise.all([
          base44.entities.Match.filter({ user_app_role: "dganit" }, '-created_date'),
  ...
  if (loading) return <div className="flex items-center justify-center min-h-[60vh]"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-violet-500"></div></div>;

  return (
    <div className="space-y-3">
      <Card className="bg-gradient-to-r from-violet-100 to-violet-50"><CardHeader className="pb-3"><CardTitle className="text-lg">דגנית - מומחית QA</CardTitle></CardHeader></Card>
