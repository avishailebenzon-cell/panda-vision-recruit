...
export default function MobileItayPage() {
    ...
    const loadData = async () => {
      try {
        const [matchesData, jobsData, candidatesData, statusesData] = await Promise.all([
          base44.entities.Match.filter({ user_app_role: "itay" }, '-created_date'),
  ...
  if (loading) return <div className="flex items-center justify-center min-h-[60vh]"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500"></div></div>;

  return (
    <div className="space-y-3">
      <Card className="bg-gradient-to-r from-indigo-100 to-indigo-50">
        <CardHeader className="pb-3"><CardTitle className="text-lg">איתי - מומחה IT</CardTitle></CardHeader>
      </Card>
        ...
        <Card className="mt-4 p-4 fixed bottom-0 left-0 right-0 rounded-t-xl shadow-2xl">
          <div className="space-y-4 pb-24">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" onClick={() => setSelectedMatch(null)} className="text-xs">סגור</Button>
            </div>
            <div className="space-y-3">
              <div><p className="text-xs font-semibold text-gray-600">מועמד</p><p className="text-sm text-gray-900">{selectedMatch.candidate_name}</p></div>
              <div><p className="text-xs font-semibold text-gray-600">משרה</p><p className="text-sm text-gray-900">{selectedMatch.job_title}</p></div>
              <div><p className="text-xs font-semibold text-gray-600">ציון התאמה</p><p className="text-2xl font-bold text-indigo-600">{Math.round(selectedMatch.match_score || 0)}%</p></div>
              {selectedMatch.match_reasons && (
                <div><p className="text-xs font-semibold text-gray-600">סיבות התאמה</p><p className="text-xs text-gray-600 mt-1">{selectedMatch.match_reasons}</p></div>
              )}
              <div className="flex gap-2 pt-4">
                <Button onClick={() => setShowSendDialog(true)} className="flex-1 bg-indigo-600 hover:bg-indigo-700">שלח</Button>
