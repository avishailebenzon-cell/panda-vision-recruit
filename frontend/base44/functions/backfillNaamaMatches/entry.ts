    ...
    try {
        ...
        for (const match of processingBatch) {
            try {
                ...
                // Construct Prompt
                const prompt = `את נעמה, מומחית גיוס תוכנה ותיקה ומקצועית.
...
הוראות:
1. נתחי את ההתאמה בין המועמד למשרה.
2. ספקי "detailed_analysis" שהוא מערך של אובייקטים. כל אובייקט ייצג דרישה אחת מהמשרה והמענה של המועמד.
   המבנה של כל אובייקט:
   - requirement: דרישת המשרה (למשל "נסיון ב-Python")
   - candidate_qualification: מה יש למועמד בהקשר זה (למשל "5 שנות נסיון בפיתוח backend")
   - is_match: האם יש התאמה בדרישה זו (true/false/partial)
3. ב-match_reasons תני סיכום קצר וקולע של ההתאמה (2-3 משפטים). כתבי בלשון נקבה.
