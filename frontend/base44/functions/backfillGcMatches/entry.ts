    ...
    try {
        ...
        for (const match of processingBatch) {
            try {
                ...
                // Construct Prompt
                const prompt = `אתה GC (Garbage Collector), סוכן כללי ומנוסה.
...
2. ספק "detailed_analysis" שהוא מערך של אובייקטים. כל אובייקט ייצג דרישה אחת מהמשרה והמענה של המועמד.
   המבנה של כל אובייקט:
   - requirement: דרישת המשרה (למשל "נסיון בתחום")
   - candidate_qualification: מה יש למועמד בהקשר זה (למשל "5 שנות נסיון בתחום")
