    ...
    try {
        ...
        for (const match of processingBatch) {
            try {
                ...
                // Construct Prompt
                const prompt = `אתה איתי, מומחה IT ותיק ומקצועי.
...
2. ספק "detailed_analysis" שהוא מערך של אובייקטים. כל אובייקט ייצג דרישה אחת מהמשרה והמענה של המועמד.
   המבנה של כל אובייקט:
   - requirement: דרישת המשרה (למשל "נסיון בניהול רשתות")
   - candidate_qualification: מה יש למועמד בהקשר זה (למשל "5 שנות נסיון בניהול רשתות ארגוניות")
   - is_match: האם יש התאמה בדרישה זו (true/false/partial)
3. ב-match_reasons תן סיכום קצר וקולע של ההתאמה (2-3 משפטים). כתוב בלשון זכר.
