...
Deno.serve(async (req) => {
  try {
    ...
    try {
      const toggles = await base44.asServiceRole.entities.AgentToggleConfig.filter({ agent_name: 'itay' });
      if (toggles.length > 0 && toggles[0].is_enabled === false) {
        console.log('⏸️ Itay is disabled via toggle - skipping run');
    ...
    const updateLog = async (message) => {
      try {
        const statuses = await base44.asServiceRole.entities.AgentRunStatus.filter({ agent_name: 'itay' });
    ...
    // Update run status
    const runStatuses = await base44.asServiceRole.entities.AgentRunStatus.filter({ agent_name: 'itay' });
    const startData = {
      agent_name: 'itay',
      is_running: true,
    ...
    // Get ONLY jobs assigned to Itay by Carmit
    ...
    // Filter ONLY jobs that Carmit assigned to Itay
    const activeJobs = allJobs.filter(j => {
      if (j.status !== 'פעילה') return false;
      return j.assigned_agent === 'itay';
    });

    // Priority order:
    // 1. Jobs with itay_priority flag (what Carmit explicitly assigned)
    // 2. Within each group - sort by recruitment_priority (1 > 2 > 3 > 4)
    // 3. Then by last processed date (oldest first)
    
    const priorityJobsExplicit = activeJobs.filter(j => j.itay_priority === true);
    const regularJobs = activeJobs.filter(j => j.itay_priority !== true);
    ...
    const sortJobsByPriority = (jobs) => {
      return jobs.sort((a, b) => {
        ...
        const aDate = a.itay_processed_date ? new Date(a.itay_processed_date).getTime() : 0;
        const bDate = b.itay_processed_date ? new Date(b.itay_processed_date).getTime() : 0;
    ...
    if (sortedJobs.length === 0) {
      await updateLog('❌ לא נמצאו משרות IT כלל');
      await base44.asServiceRole.entities.AgentRunStatus.update(runStatuses[0].id, {
        is_running: false,
        last_run_end: new Date().toISOString(),
        matches_created: 0
      });
      return Response.json({ success: true, matchesCreated: 0, message: 'אין משרות IT' });
    }

    await updateLog(`✅ ${sortedJobs.length} משרות IT לעיבוד (${priorityJobsExplicit.length} בעדיפות)`);
    ...
    for (let jobIndex = 0; jobIndex < Math.min(sortedJobs.length, MAX_JOBS_PER_RUN); jobIndex++) {
      ...
      await updateLog(`📋 ${candidatesToProcess.length} מועמדים חדשים (${jobExistingMatches.length} נבדקו)`);
      ...
      for (let batchStart = 0; batchStart < candidatesToProcess.length; batchStart += 30) {
        ...
        const isPriorityJob = job.itay_priority === true;
        const priorityNote = isPriorityJob ? '\n🔴 עדיפות!' : '';

        // Load feedback context for this agent
        let agentFeedbackText = '';
        try {
          const feedbackResp = await base44.asServiceRole.functions.invoke('getAgentFeedbackContext', { agent_name: 'itay' });
          agentFeedbackText = feedbackResp?.data?.feedbackText || '';
        } catch (e) { /* ignore */ }

        const prompt = `אתה איתי, סוכן גיוס מומחה IT ותשתיות טכנולוגיות של חברת פנדה-טק. דבר תמיד בלשון זכר.
...
- ⛔ תחום לא רלוונטי: אם main_discipline הוא אדריכלות/בנייה/עיצוב פנים/אינסטלציה/סולארי/אזרחית ואין ניסיון IT תעשייתי מוכח → אל תחזיר בכלל
- ⛔ HARD GATE - ניסיון אקדמי בלבד: אם אין אף תפקיד תעשייתי ממשי (IT Engineer/Admin/Analyst בחברה) → אל תחזיר בכלל
- ⛔ HARD GATE - בוגר טרי: אם years_experience ≤ 1 והמשרה דורשת "X שנות ניסיון" (X>1) → אל תחזיר בכלל

כללים לא סחירים:
1) IT ותשתיות טכנולוגיות בלבד - System/Network/Cloud/SecOps/DBA/Helpdesk/NOC/IT Support, וגם מערכות מיגון (Protection Systems), כתיבת מפרט דרישות מערכת, פיקוח על מבחני קבלה. תוכנה/אלקטרוניקה/הנדסת מערכת ארכיטקטונית לא בתחום.
...
7) ציון 90+ נדיר - רק אם 0 חוסרי Core + כל Hard Gates + כישורים מוכחים (לא "מילה ברשימה")
8) סף הצגה: 70+ בלבד
9) מרחק >100 ק"מ → Hard Gate (69 ומטה)
10) Hands-on - מועמדים ניהוליים בעיקרם ללא עשייה → הורד משמעותית
11) ⛔ ניסיון תעשייתי: ספור רק IT Engineer/Admin/Analyst בחברות. אל תספור: שירות צבאי, פרויקטים אקדמיים
...
עקרון ראיות: טכנולוגיה/כישור "קיימים" רק עם ניסיון מוכח בתפקיד/אחריות/פרויקט אופרטיבי. לא נחשב: skills list, אזכור יחיד, "היכרות".

קו"ח ישנים (>3 שנים): הורד 10-15 נקודות + הוסף "⚠️ קורות חיים מ-[שנה]".
${agentFeedbackText}

הוראות פלט:
1. נתח משרה: Hard Gates, Core Skills (3-6), Secondary, קלסטר IT
2. בדוק מועמדים: Hard Gates? Core מוכחים? טכנולוגיות ספציפיות מוכחות?
3. החזר רק 70+
4. detailed_analysis מלא: requirement, candidate_qualification, is_match
5. match_reasons (זכר): Hard Gates, Core Skills (מוכח/חלש/חסר + איפה), טכנולוגיות, קלסטר, ניסיון, הסמכות, פערים
...
החזר עד 5 מועמדים.`;
      ...
      await base44.asServiceRole.entities.Job.update(job.id, {
        itay_processed_date: new Date().toISOString(),
        itay_priority: false
      });
    ...
    await updateLog(`🎯 איתי סיים! ${totalMatchesCreated} התאמות ב-${sortedJobs.length} משרות`);
    ...
    if (totalMatchesCreated > 0) {
      await base44.asServiceRole.entities.SystemActivityLog.create({
        ...
        action_description: `איתי עבר על ${sortedJobs.length} משרות IT ומצא ${totalMatchesCreated} התאמות`,
        status: 'success'
      });
    }
    ...
    try {
      ...
      const runStatuses = await base44Fallback.entities.AgentRunStatus.filter({ agent_name: 'itay' });
      if (runStatuses && runStatuses.length > 0) {
